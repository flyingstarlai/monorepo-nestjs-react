...
export class SqlEditorController {
  ...
  async getProcedures(
    ...
    const { workspace } = req;
    const procedures = await this.sqlEditorService.getProceduresForWorkspace(
      workspace.id
    );
    return procedures;
  }
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - procedure name already exists',
  })
  async createProcedure(
    ...
    return await this.sqlEditorService.createProcedure(
      workspace.id,
      createDto,
      user.id
    );
  ...
  async getProcedure(
    ...
    const { workspace } = req;
    return await this.sqlEditorService.getProcedureById(
      procedureId,
      workspace.id
    );
  }
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - cannot update this procedure',
  })
  async updateProcedure(
    ...
    const { user, workspace } = req;
    return await this.sqlEditorService.updateProcedure(
      procedureId,
      workspace.id,
      updateDto,
      user.id
  ...
  @ApiResponse({
    status: 403,
    description: 'Forbidden - cannot delete this procedure',
  })
  async deleteProcedure(
    ...
    const { user, workspace } = req;
    await this.sqlEditorService.deleteProcedure(
      procedureId,
      workspace.id,
      user.id
    );
  ...
  @ApiResponse({
    status: 201,
    description: 'Procedure duplicated successfully',
    type: StoredProcedureResponseDto,
  })
  @ApiResponse({ status: 404, description: 'Procedure not found' })
  ...
  @ApiResponse({
    ...
    type: PublishProcedureResponseDto,
  })
  @ApiResponse({ status: 404, description: 'Procedure not found' })
  @ApiResponse({
    status: 400,
    description: 'Bad request - cannot publish draft',
  })
  async publishProcedure(
    @Param('slug') slug: string,
    @Param('procedureId') procedureId: string,
    @Body() publishDto: PublishProcedureDto,
    @Request() req: { user: User; workspace: any }
  ) {
    const { user, workspace } = req;
  ...
  @ApiResponse({
    status: 200,
    description: 'Procedure unpublished successfully',
    type: StoredProcedureResponseDto,
  })
  @ApiResponse({ status: 404, description: 'Procedure not found' })
  @ApiResponse({
    status: 400,
    description: 'Bad request - procedure is not published',
  })
  async unpublishProcedure(
    ...
    const { user, workspace } = req;
    return await this.publishService.unpublishProcedure(
      procedureId,
      workspace.id,
      user.id
    );
  ...
  @ApiResponse({
    ...
    type: ProcedureExecutionResponseDto,
  })
  @ApiResponse({ status: 404, description: 'Procedure not found' })
  @ApiResponse({
    status: 400,
    description: 'Bad request - cannot execute draft',
  })
  ...
  async validateProcedure(
    ...
    const { workspace } = req;
    return await this.validationService.validateDraft(
      procedureId,
      workspace.id
    );
  }
