...
Monorepo for a modern dashboard with a NestJS API and a React web app. It delivers JWT authentication, role-based access control (Admin/User), and an Admin Panel to create users, enable/disable accounts, and change roles. Shared tooling (TypeScript, ESLint/Prettier/Biome, Jest/Vitest) and Turborepo keep code style, testing, and builds consistent and fast.

## Tech Stack

- Monorepo: PNPM workspaces, Turborepo 2
- Language/Runtime: TypeScript 5, Node >= 18
- Backend: NestJS 11, TypeORM 0.3, Passport (JWT), class-validator/transformer, bcrypt; DB: SQLite (dev default) with optional Postgres
- Frontend: React 19, Vite 7, TanStack Router v1 (file-based via plugin), TanStack Query v5, TanStack Table v8, Tailwind CSS v4, Shadcn UI (Radix UI), React Hook Form + Zod, Sonner toasts; env-based API URL via `VITE_API_BASE_URL`
- Testing: Jest 29 for API (unit + e2e), Vitest 3 + Testing Library for web
- Lint/Format: Biome 2 everywhere with shared config (@repo/biome-config); ESLint/Prettier deprecated
...
- Database: SQLite file (`apps/api/data.sqlite`) in dev with `synchronize: true`; entities `User` and `Role` (eager relation). Seeds ensure roles (Admin/User) and a default admin/user
...
- API boundaries: REST endpoints under `/auth` and `/users`; server returns sanitized user shapes (no password); consistent Nest exceptions map to client errors
...
- Node.js >= 18
- Dev DB: SQLite file with `synchronize: true` (do not use `synchronize` in production); Postgres supported via envs if configured
